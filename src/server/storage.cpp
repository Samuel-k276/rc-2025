...
void save_event_start(int eid, const std::string& uid, const std::string& name, 
    ...
    if (file.is_open()) {
        file << uid << " " << name << " " << desc_fname << " " 
             << event_attend << " " << start_date << " " << start_time;
        file.close();
    }
}
            ...
            if (file.is_open()) {
                ...
                if (file >> uid_from_file >> res_num >> date_part >> time_part) {
                    ...
                    for (int eid : all_eids) {
                        std::string eid_str = format_eid(eid);
                        std::string event_reserv_file = "src/server/storage/EVENTS/" + eid_str + "/RESERVATIONS/" + filename;
                        struct stat st;
